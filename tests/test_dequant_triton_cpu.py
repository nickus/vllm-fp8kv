...
# Under the interpreter these run on CPU; on a real GPU the launch needs device
# tensors. Same test either way — that is the point.
BYTES = ALL_BYTES.to(DEVICE)


def _ref(dtype):
    return BYTES.view(torch.float8_e4m3fn).to(dtype)


def test_triton_bitmath_and_lut_all_256_bytes():
    lut = build_lut(torch.float16, device=DEVICE)
    out_bm, out_lut = dequant_triton_launch(BYTES, lut)
...
def test_triton_lut_bf16_table():
    lut = build_lut(torch.bfloat16, device=DEVICE)
    _, out_lut = dequant_triton_launch(BYTES, lut)
...
def test_triton_decode_masks_the_tail_of_a_ragged_buffer():
    ...
    u8 = torch.randint(0, 256, (300,), generator=g, dtype=torch.uint8).to(DEVICE)
    lut = build_lut(torch.float16, device=DEVICE)
...
def test_denormals_and_signed_zero_survive_the_bitmath():
    ...
    lut = build_lut(torch.float16, device=DEVICE)
    denormals = torch.tensor([0x01, 0x02, 0x07, 0x80, 0x81, 0x87],
